...
legume_crops = df_2024_2030[df_2024_2030['作物类型'].isin(['粮食（豆类）', '蔬菜（豆类）'])]['作物编号'].unique()
...
for iteration in range(num_iterations):
    ...
    for year in years:
        for land_id in df_sheet_1['地块名称'].unique():
            available_area = df_sheet_1[df_sheet_1['地块名称'] == land_id]['地块面积/亩'].values[0]
            crop_areas = np.zeros(len(df_2024_2030['作物编号'].unique()))

            # 1. 随机分配种植面积，确保不超过可用面积
            ...
            if year % 3 == 0 and not any(df_2024_2030['作物编号'].isin(legume_crops)):
                ...
                crop_areas[legume_idx] = max(5, np.random.uniform(0, available_area)) 
            ...
            # 5. 根据地块类型适应性分配作物
            land_type = df_sheet_1[df_sheet_1['地块名称'] == land_id]['地块类型'].values[0]
            for i, crop_id in enumerate(df_2024_2030['作物编号'].unique()):
                crop_type = df_2024_2030[df_2024_2030['作物编号'] == crop_id]['作物类型'].values[0]
            ...
            for crop_id, area in zip(df_2024_2030['作物编号'].unique(), crop_areas):
                ...
                if total_yield <= sales:
                    ...
                    if situation == 1:
                        ...
                        total_revenue += sales * sales_price + excess * sales_price * 0.5 
