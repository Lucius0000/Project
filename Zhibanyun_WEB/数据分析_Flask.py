...
try:
    ...
    #dfuf.to_excel("UsedFreq_cleaned_excel_file.xlsx", index = False)
    ...
    #dfkm.to_excel("kmeansdata_excel_file.xlsx", index = False)
...
factor_path = os.path.join(output_path,"all_factor_assignments.xlsx")
factor_df.to_excel(factor_path, index=False)
...
plt.tight_layout()
plt.savefig('功能需求分析的线性回归结果（对话自然度）', dpi = 300)
##plt.show()
...
plt.tight_layout()
plt.savefig('功能需求分析的线性回归结果（情感支持）', dpi = 300)
##plt.show()
...
plt.tight_layout()
plt.savefig('功能需求分析的线性回归结果（隐私保护）', dpi = 300)
##plt.show()    
...
plt.tight_layout()
plt.savefig('功能需求分析的线性回归结果（个性化定制功能）', dpi = 300)
##plt.show()
...
# 保存并展示
plt.tight_layout()
plt.savefig("整体体验_回归结果.png", dpi=300)
##plt.show()
...
plt.tight_layout()
plt.savefig('付费意愿分析的回归结果.png', dpi=300)
##plt.show()
...
# 保存并展示
plt.tight_layout()
plt.savefig("使用频率_回归结果.png", dpi=300)
##plt.show()
...
dfkm.to_excel("聚类结果.xlsx", index=False)
...
plt.colorbar(label='Cluster')
plt.savefig('KMeans_tSNE_2D.png')
##plt.show()
plt.close()
...
print("模型参数估计结果：")
print(params)
params.to_excel("SEM模型参数.xlsx", index=False)

# 计算拟合度
fit = calc_stats(model) 
print("\n拟合度指标：")
print(fit.T)
fit.to_excel("SEM拟合度.xlsx", index=False)
...
# 输出图像
dot.render(filename = 'Vertical_SEM_Path_Diagram', format='png', cleanup=True)

print("数据处理已结束，请点击下载结果")
