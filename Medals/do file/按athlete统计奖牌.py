# -*- coding: utf-8 -*-
"""
Created on Sat Jan 25 16:48:28 2025
...
df_gold = df_gold.groupby(['year', 'noc'])['sport_event'].nunique().reset_index()
...
df_silver = df_silver.groupby(['year', 'noc'])['sport_event'].nunique().reset_index()
...
df_bronze = df_bronze.groupby(['year', 'noc'])['sport_event'].nunique().reset_index()
...
df_combined = pd.merge(df_gold, df_silver, on=['year', 'noc'], how='outer')
df_combined = pd.merge(df_combined, df_bronze, on=['year', 'noc'], how='outer')
...
df_combined.to_stata('re_medal.dta', write_index=False)